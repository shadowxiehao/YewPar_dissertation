...
This paper analyzes the workflow of workers under the YewPar framework, as shown in \ref{fig:period_of_worker}.
...
we have designed a comprehensive data collection scheme as depicted in \ref{fig:worker_collect_calculate}.
...
\begin{itemize}
    ...
    \item A deeper investigation into the application's implementation and specific test data suggested the primary issue likely stemmed from performance anomalies referenced in \ref{sec:performance_anomalies}.
          ...
          it's worth noting in \ref{fig:time_comparison_max_solo} that between 15 and 20 nodes,
...
For the NS-hivert program based on the Budget skeleton, the results after averaging the runtimes from ten iterations for each test set are depicted in \ref{fig:time_comparison_ns_solo}.
...
We also analyzed the runtime variability of the NS-hivert program, as shown in \ref{fig:NS_hivert_fluctuations}.
